...
*/
__kernel oobp3_full_down_1_U${U}_X${X}_UU${UU}_XU${XU}(
...
)
{
    ...
    if (kWIRow == 0)
    {
        ...
        event_t e_copy = async_work_group_copy(lo_u_rho, g_x_rho_offset, ${XU}, 0);
    ...
    for (int j = 0; j < ${XU}; j ++)
    {
        float a = lo_u_rho[n] / g_w_lambda_offset[j];
        sum += a;
        w_pi[j] = a;
    }
    for (int j = 0; j < ${XU}; j ++)
